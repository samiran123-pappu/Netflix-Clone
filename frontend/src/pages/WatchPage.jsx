...
const WatchPage = () => {
    const { id } = useParams()
    const [trailers, setTrailers] = useState([]);//empty array
    const [loading, setLoading] = useState(true);
    const [currentTrailerIdx, setCurrentTrailerIdx] = useState(0);
    const [content, setContent] = useState({});//empty object
        ...
        const getTrailers = async () => {
            try {
                ...
                if (error.message.includes("404")) {
                    setTrailers([]); // Fallback to empty array if undefined
                }
    ...
    useEffect(() => {
        const getSimilarContent = async () => {
            try {
                ...
                if (error.message.includes("404")) {
                    setSimilarContent([]); // Fallback to empty array if undefined
                }
            }
        }
        getSimilarContent();
    ...
    return <div className="bg-black min-h-screen text-white">
        <div className="mx-auto container px-4 py-8 h-full">
            ...
            >
                ...
                <img src={LARGE_IMAGE_BASE_URL + content?.poster_path} alt="Poster Image" className="max-h-[600px] rounded-md" />
